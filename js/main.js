//PRESENTER:
define(["view", "model", "factory", "underscore", "events"], function (
  view,
  model,
  factory,
  _,
  ...
  var getNewNotenDate = function () {
    return new Date().toJSON().substr(2, 14).replace("T", " ");
  };
  ...
  var noteManager = {
      ...
      if (event.type === "up" || event.type === "undo") {
        ...
        var note = (function (model, undoEvent) {
          if (event.type === "undo") {
            ...
            model.historyStack.push({
              name: "undoCreate",
              id: newNote.noteID.toString(),
              type: "undo",
            });
            return newNote;
          }
      ...
      if (exec.textArea) {
        ...
        if (textArea.tagName === "TEXTAREA") {
          ...
          textArea.stopedTyping = setTimeout(function () {
            ...
            model.historyStack.push({
              name: "undoEdit",
              noteID: savedNote.noteID.toString(),
              prevText: savedNote.noteText,
              prevMod: savedNote.modInfo,
              type: "undo",
            });
      ...
      if (event.type === "down" || event.type === "undo") {
        ...
        if (event.type !== "undo") {
          ...
          event.target.ondragend = function (eventEnd) {
            ...
            model.savedNotes[target.id].noteLeft = target.style.left.substr(
              0,
              5
            );
    ...
    moveBack: function (event) {
      ...
      events.trigger("changeOnNotes", {
        ...
        initialLeft: event.initialLeft,
        initialTop: event.initialTop,
      });
    },
    undo: function (event) {
      var key = event.key;
    ...
    execute: function (exec) {
      return exec.key === "z" && exec.ctrlKey
        ...
        : noteManager[exec.name] &&
            noteManager[exec.name].apply(noteManager, arguments);
    },
  };
